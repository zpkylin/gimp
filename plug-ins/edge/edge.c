...
init_tile_buf( TileBuf *buf, GDrawable *drawable )
{
  ...
  buf->col = 0;
  buf->row = 0;
  buf->bpp = gimp_drawable_bpp (drawable->id);
...
edge( GDrawable *drawable )
{
  ...
  num_channels = alpha = gimp_drawable_num_channels ( drawable->id );
  ...
  for( pr = gimp_pixel_rgns_register( 2, &src_rgn, &dest_rgn ); 
	    ...
	    y++, srcrow += src_rgn.rowstride, destrow += dest_rgn.rowstride )
	{
	  switch(precision)
	  {
	      case PRECISION_U8:
		{
		  guint8 *src = (guint8*)srcrow;
		  guint8* dest = (guint8*)destrow;

		  guint8 pix00[3], pix01[3], pix02[3];
		  guint8 pix10[3], pix12[3];
		  guint8 pix20[3], pix21[3], pix22[3];
  		  gint maxval = 255;
		  glong sum1, sum2;
		  glong sum;

		  gint rowstride = src_rgn.rowstride;  /*the total number of channels in this row*/
		  for( x = dest_rgn.x;
		       x < (dest_rgn.x + dest_rgn.w);
		       x++,  src += num_channels, dest += num_channels )
		    {
...
#define PIX(X,Y)  src[ (Y-1)*rowstride + (X-1)*num_channels + chan ]
...
#undef  PIX
			      /* common job ... */
			      sum = sqrt ( (gdouble) sum1 * sum1 + (gdouble) sum2 * sum2);

			      sum *= scale;    /* scaling factor in [1,10]*/
		...
		break;
	      case PRECISION_U16:
		{
		  guint16 *src = (guint16*)srcrow;
		  guint16* dest = (guint16*)destrow;

		  guint16 pix00[3], pix01[3], pix02[3];
		  guint16 pix10[3], pix12[3];
		  guint16 pix20[3], pix21[3], pix22[3];
  		  gint maxval = 65535;
		  glong sum1, sum2;
		  glong sum;

		  gint rowstride = src_rgn.rowstride / 2;  /*the total number of channels in this row*/
		  for( x = dest_rgn.x;
		       x < (dest_rgn.x + dest_rgn.w);
		       x++,  src += num_channels, dest += num_channels )
		    {
...
#undef  PIX
			  ...
			  */
			   ...
			   * right work with `wrapmode'.
			   */
			  get_tile_pixel( &buf, x-1, y-1, (guchar*)pix00, wrapmode );
			  get_tile_pixel( &buf, x  , y-1, (guchar*)pix10, wrapmode );
			  get_tile_pixel( &buf, x+1, y-1, (guchar*)pix20, wrapmode );
			  get_tile_pixel( &buf, x-1, y  , (guchar*)pix01, wrapmode );
			  get_tile_pixel( &buf, x+1, y  , (guchar*)pix21, wrapmode );
			  get_tile_pixel( &buf, x-1, y+1, (guchar*)pix02, wrapmode );
			  get_tile_pixel( &buf, x  , y+1, (guchar*)pix12, wrapmode );
			  get_tile_pixel( &buf, x+1, y+1, (guchar*)pix22, wrapmode );
		...
		{
		  gfloat *src = (gfloat*)srcrow;
		  gfloat* dest = (gfloat*)destrow;

		  gfloat pix00[3], pix01[3], pix02[3];
		  gfloat pix10[3], pix12[3];
		  gfloat pix20[3], pix21[3], pix22[3];
  		  gint maxval = 1.0;
		  gfloat sum1, sum2;
		  gfloat sum;

		  gint rowstride = src_rgn.rowstride/4;  /*the total number of channels in this row*/
		  for( x = dest_rgn.x;
		       x < (dest_rgn.x + dest_rgn.w);
		       x++,  src += num_channels, dest += num_channels )
		    {
...
#undef  PIX
			  ...
			  */
			  ...
			  for( chan = 0; chan < alpha; chan++ )
			    {
			      /* make convolusion */
			      sum1 = (FLT(pix20[chan], u[0]) - FLT(pix00[chan], u[1]) ) +
				 2 * (FLT(pix21[chan], u[2]) - FLT(pix01[chan], u[3]) ) +
				     (FLT(pix22[chan], u[4]) - FLT(pix02[chan], u[5]) );

			      sum2 = (FLT(pix02[chan], u[0]) - FLT(pix00[chan], u[1]) ) +
				 2 * (FLT(pix12[chan], u[2]) - FLT(pix10[chan], u[3]) ) +
				     (FLT(pix22[chan], u[4]) - FLT(pix20[chan], u[5]) );
