		  ...
		  gint init_mode)
{
  ...
  bsp = g_malloc (sizeof (BrushSelect));
  bsp->redraw = TRUE;
  bsp->scroll_offset = 0;
  bsp->callback_name = 0;
  ...
  /* The noise toggle */
  noise_button = gtk_check_button_new_with_label ("Noise");                                          
  ...
  label = gtk_label_new ("Noise Freq:");
  gtk_box_pack_start (GTK_BOX (util_box), label, FALSE, FALSE, 2);
#if 0
  bsp->noise_freq_data = 
...
display_brush_get_row_funcs (Tag t)
{
  switch (tag_precision (t))
    ...
    case PRECISION_NONE:
    default:
      g_warning ("display_brush_get_row_funcs: bad precision");
    }
  return NULL;
}
...
edit_brush_callback (GtkWidget *w, GdkEvent *e, gpointer data)
{
  if (GIMP_IS_BRUSH_GENERATED(get_active_brush()))
  {
    ...
    g_message("Active brush is not generated\n");
  return TRUE;
}
